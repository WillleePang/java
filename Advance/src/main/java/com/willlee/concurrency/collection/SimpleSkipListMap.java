 ...
 * 
 *         4.在remove节点时，直接将value赋值为null，然后在后续操作中慢慢删除节点和其索引。
 *         这种思想是与ConcurrentSkipListMap相同的，
 *         但ConcurrentSkipListMap是将value赋值为null的同时，在这个节点后还插入一个标志节点；
 * @param <K>
 * @param <V>
 */
...
public class SimpleSkipListMap<K, V> extends AbstractMap<K, V> implements Cloneable, java.io.Serializable {
    ...
    private static final long serialVersionUID = 6422383292906447753L;
    ...
    /**
     * 节点类，注意两点：1.value为null时，表示此节点已经删除；2.value的类型为Object是因为一些特殊情况它不为V类型，
     * 例如BASE_HEADER
     */
    ...
    private int randomLevel() {
        ...
        x ^= x >>> 17;
        randomSeed = x ^= x << 5;
        if ((x & 0x80000001) != 0) // 如果最高位和最低位其中有一个或者两者都为1，那么表示为极端情况，则直接抛弃这种情况
            return 0;
        int level = 1;
        while (((x >>>= 1) & 1) != 0)// 用0和1的一串二进制来代表投掷硬币的结果集，随着不断的向右移，如果末尾一位是0，那么立即结束，累加的level就是当前的期望值
            ++level;
        return level;
    }
